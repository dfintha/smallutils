...
# --- macOS: OSAScript ------------------------------------------------------- #
    ...
    async def _fetch() -> tuple:
        process = subprocess.run(
            [
                ...
                "if player state is playing then artist of current track"
            ],
            capture_output=True,
            check=False,
            encoding="utf-8",
        ...
        process = subprocess.run(
            [
                ...
                "-e",
                "tell application 'Spotify' to "
                "if player state is playing then name of current track"
            ],
            capture_output=True,
            check=False,
            encoding="utf-8",
    ...
    def _mute(mute: bool) -> None:
        mode = "without" if mute else "with"
        subprocess.run(
            ["osascript", "-e", f"set volume {mode} output muted"],
            capture_output=True,
            check=False,
            encoding="utf-8",
        )
